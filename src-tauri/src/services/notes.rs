...
/// 兼容旧版环境变量名：新版本优先读 FASTNOTE_*，其次回退 FLORAL_NOTEPAPER_*
fn env_dir_value(primary: &str, legacy: &str) -> Option<PathBuf> {
    for var in [primary, legacy] {
...
pub(crate) fn default_config_dir() -> Result<PathBuf, AppError> {
    if let Some(path) = env_dir_value("FASTNOTE_CONFIG_DIR", "FLORAL_NOTEPAPER_CONFIG_DIR") {
        return Ok(path);
    }
    if let Some(dir) = dirs::config_dir() {
        return Ok(dir.join("fastnote"));
    }
    Ok(env::current_dir()?.join("fastnote"))
}

fn default_data_dir() -> Result<PathBuf, AppError> {
    if let Some(path) = env_dir_value("FASTNOTE_DATA_DIR", "FLORAL_NOTEPAPER_DATA_DIR") {
        return Ok(path);
    }
...
mod tests {
    ...
    fn test_root(name: &str) -> PathBuf {
        let base = std::env::var_os("FASTNOTE_TEST_TEMP_DIR")
            .or_else(|| std::env::var_os("FLORAL_NOTEPAPER_TEST_TEMP_DIR"))
            .map(PathBuf::from)
            .unwrap_or_else(|| std::env::temp_dir().join("fastnote-rust-tests"));
    ...
    fn test_store(name: &str) -> NoteStore {
        ...
        // developer's real fastnote data into the test directory on macOS.
